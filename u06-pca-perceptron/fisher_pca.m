function meanSuccessRate = fisher_pca(dim)
  ...
  pcs = principalComponents(tmp);
  tmp = transformData(pcs, tmp, dim);
  tra = [tmp tra(:,end)];
  ...
  dlmwrite('fisher_results_pca.mat', result, ' ');
...
function r = principalComponents(data)
  ...
  covarMatrix = covarMatrix/norm(covarMatrix);
